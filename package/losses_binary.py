...
class RecallatK_binary(torch.nn.Module):
    def __init__(self, anneal,  k_vals, k_temperatures):

        """
            Parameters
            _______________________

            anneal : parameter for sigmoid for soft rank
            k_vals : k values
            k_temperatures: parameter for sigmoid for soft recall
        
        """

        super(RecallatK_binary, self).__init__()
        self.anneal = anneal
        self.k_vals = k_vals
        self.k_temperatures = k_temperatures
    ...
    def forward(self, y_true, preds):
        ...
        """
            ...
            preds: torch.Tensor
                prediction of model
            y_true: torch.Tensor
                binary label of batch

        """

        q_id = [True if a == 1 else False for a in y_true]

        # calc sigmoid rank for all positive smaples
        """
                pred ...
            pred
                |
